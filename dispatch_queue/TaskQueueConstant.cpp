...
#include <cstdint>
#include <chrono>
#include "pipeline/PipelineABConfig.h"
namespace task
{
...
{
    sMaxTaskQueueCount = pipeline::PipelineABConfig::GetInstance().getMaxTaskQueueCount();
    sMaxReportCountThreshold = pipeline::PipelineABConfig::GetInstance().getMaxReportCount();
    sMaxSleepTimeout = std::chrono::seconds(pipeline::PipelineABConfig::GetInstance().getMaxWaitDestory());
    sBlockTimeoutThreshold = pipeline::PipelineABConfig::GetInstance().getBlockTimeoutThreshold();
}
