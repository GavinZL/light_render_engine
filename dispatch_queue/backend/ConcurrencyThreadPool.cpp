...
void ConcurrencyThreadPool::execute(const TaskOperatorPtr& task, TaskQueuePriority priority)
{
    ...
    {
        LOGE("[TASK]ConcurrencyThreadPool::execute, invalid priority: %d", priority);
        assert(false);
        return;
    }
...
void ConcurrencyThreadPool::_schedule(TaskQueuePriority priority)
{
    ...
    if (activeCount < mData->mMaxThreads.load(std::memory_order_acquire))
    {
        ...
        LOGE("[TASK]ConcurrencyThreadPool::_schedule, create new thread, threadId: %d, idleThreads: %d, activeThreads: %d", thread->threadId(), idleCount, activeCount);
    }
    else
    {
