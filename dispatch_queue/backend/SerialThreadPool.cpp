...
int32_t SerialThreadPool::attachOneThread(const std::string& name, WorkThreadPriority prio)
{
    int32_t index = -1;
    {
        ...
        for (auto it = mExclusiveThreads.begin(); it != mExclusiveThreads.end();)
        {
            ...
            if (it->second && it->second->isBlocked())
            {
                ...
                LOGE("[TASK]SerialThreadPool::attachOneThread, threadId: %d is blocked, blockedInfo: %s", it->second->threadId(), it->second->blockedInfo().c_str());
                // 尝试cancel 与 删除
                it->second->cancel();
    ...
    if (index == -1)
    {
        ...
        {
            LOGE("[TASK]SerialThreadPool::attachOneThread, reach max thread count, max: %d", SysUtils::cpuCount());
        ...
        LOGE("[TASK]SerialThreadPool::attachOneThread, create new thread, threadId: %d", index);
    }

    LOGE("[TASK]SerialThreadPool::attachOneThread, name: %s, prio: %d, threadId: %d", name.c_str(), prio, index);
