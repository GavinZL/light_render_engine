...
public:
    ...
    {
        auto task = std::make_shared<TaskOperator>(mark, [func](const TaskOperatorPtr&) {
            func();
        });
        async(task);
    }
    ...
    void sync(const TaskMark& mark, Func&& func, std::chrono::milliseconds timeout = std::chrono::milliseconds(-1))
    {
        auto task = std::make_shared<TaskOperator>(mark, [func](const TaskOperatorPtr&) {
            func();
        });
    ...
    void after(const TaskMark& mark, std::chrono::milliseconds delay, Func&& func)
    {
        auto task = std::make_shared<TaskOperator>(mark, [func](const TaskOperatorPtr&) {
            func();
        });
