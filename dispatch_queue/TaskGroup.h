...
public:
    ...
    void asyncQueue(const TaskMark& mark, Func&& f, const TaskQueuePtr& queue = nullptr)
    {
        auto task = std::make_shared<TaskOperator>(mark, [f](const TaskOperatorPtr&) {
            f();
        });
    ...
    void async(const TaskMark& mark, Func&& f, TaskQueuePriority priority = TaskQueuePriority::TQP_Normal)
    {
        auto task = std::make_shared<TaskOperator>(mark, [f](const TaskOperatorPtr&) {
            f();
        });
    ...
    void notify(const TaskMark& mark, Func&& f, const TaskQueuePtr& queue = nullptr)
    {
        auto task = std::make_shared<TaskOperator>(mark, [f](const TaskOperatorPtr&) {
            f();
        });
