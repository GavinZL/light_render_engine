 ...
 * @brief OpenGL命令缓冲区实现
 * 
 * OpenGL是即时渲染模式，所以这个实现主要是记录命令并立即执行
 * 或者缓存命令用于后续重放和优化
 */
class CommandBufferGL : public HYCommandBuffer {
public:
    CommandBufferGL();
    virtual ~CommandBufferGL();
...
/**
 * @brief OpenGL ES命令缓冲区实现（继承自OpenGL实现）
 */
class CommandBufferGLES : public CommandBufferGL {
public:
    CommandBufferGLES() = default;
    virtual ~CommandBufferGLES() = default;
    
    // 可以根据需要重写特定的方法来处理OpenGL ES的差异
};
